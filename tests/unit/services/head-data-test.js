...
module('Unit | Service | head data', function (hooks) {
  ...
  test('title with currentPage and currentSelect details', function (assert) {
    ...
    assert.equal(
      headDataService.title,
      'Ember Guides',
      'Default title without currentPage nor currentSelection'
    );
    ...
    assert.equal(
      headDataService.title,
      'Ember Guides',
      'Default title without currentSelection'
    );
    ...
    assert.equal(
      headDataService.title,
      'CurrentPage title - CurrentSection title - Ember Guides',
      'Title with currentPage and currentSelection'
    );
  });
});
