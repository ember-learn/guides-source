...
module('Acceptance | meta data', function (hooks) {
  ...
  test('meta data title and description', async function (assert) {
    ...
    assert.equal(title, 'Defining Your Routes - Routing - Ember Guides');

    const descriptionStart =
      'When your application starts, the router matches the current URL to the routes';
    assert.ok(
      description.content.startsWith(descriptionStart),
      `${description.content} should start with ${descriptionStart}`
    );
  });
});
