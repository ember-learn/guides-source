...
The other one is [`replaceWith()`](https://api.emberjs.com/ember/3.18/classes/Route/methods/transitionTo?anchor=replaceWith/) which works the same way as `transitionTo()`.
...
Since a route's [`beforeModel()`](https://api.emberjs.com/ember/3.18/classes/Route/methods/transitionTo?anchor=beforeModel) executes before the `model()` hook,
...
If you need information about the current model in order to decide about redirection, you can use the [`afterModel()`](https://api.emberjs.com/ember/3.18/classes/Route/methods/transitionTo?anchor=afterModel) hook.
...
Instead, we can use the [`redirect()`](https://api.emberjs.com/ember/3.18/classes/Route/methods/transitionTo?anchor=redirect) method, which will leave the original
