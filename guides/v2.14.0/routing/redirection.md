...
One of the methods is [`transitionTo()`](https://api.emberjs.com/classes/Ember.Route.html#method_transitionTo).
Calling `transitionTo()` from a route or
[`transitionToRoute()`](https://api.emberjs.com/classes/Ember.Controller.html#method_transitionToRoute) from a controller will stop any transitions currently in progress and start a new one, functioning as a redirect.
`transitionTo()` behaves exactly like the [link-to](../../templates/links/) helper.

The other one is [`replaceWith()`](https://api.emberjs.com/classes/Ember.Route.html#method_replaceWith) which works the same way as `transitionTo()`.
...
Since a route's [`beforeModel()`](https://api.emberjs.com/classes/Ember.Route.html#method_beforeModel) executes before the `model()` hook,
...
If you need information about the current model in order to decide about redirection, you can use the [`afterModel()`](https://api.emberjs.com/classes/Ember.Route.html#method_afterModel) hook.
...
Instead, we can use the [`redirect()`](https://api.emberjs.com/classes/Ember.Route.html#method_redirect) method, which will leave the original
