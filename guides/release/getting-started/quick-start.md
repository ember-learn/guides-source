...
The component tag name must start with capital letters if component name is single worded like `components/navbar.hbs` then component tag will be `<Navbar ... />` if component name is multiple worded like `people-list` use `<PeopleList ... />` as tag name.
```
