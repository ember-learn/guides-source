...
One of the methods is [`transitionTo()`](https://api.emberjs.com/ember/4.6.0/classes/RouterService/methods/transitionTo?anchor=transitionTo).
...
The other one is [`replaceWith()`](https://api.emberjs.com/ember/4.6.0/classes/RouterService/methods/replaceWith?anchor=replaceWith) which works the same way as `transitionTo()`.
...
Since a route's [`beforeModel()`](https://api.emberjs.com/ember/4.6.0/classes/Route/methods/beforeModel?anchor=beforeModel) executes before the `model()` hook,
...
If you need information about the current model in order to decide about redirection, you can use the [`afterModel()`](https://api.emberjs.com/ember/4.6.0/classes/Route/methods/afterModel?anchor=afterModel) hook.
...
Instead, we can use the [`redirect()`](https://api.emberjs.com/ember/4.6.0/classes/Route/methods/redirect?anchor=redirect) method, which will leave the original
