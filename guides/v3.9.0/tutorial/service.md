...
To instantiate the service, we can instantiate it through ember's resolver using the [`factoryFor`](https://api.emberjs.com/ember/3.9/classes/ApplicationInstance/methods/factoryFor?anchor=factoryFor) method.
...
For cases where we do not need to override parts of the service, we can use [`lookup`](https://api.emberjs.com/ember/3.9/classes/ApplicationInstance/methods/lookup?anchor=lookup)
...
The location in the cache has been [`camelized`](https://api.emberjs.com/ember/3.9/classes/String/methods/camelize?anchor=camelize) (line 38),
...
Then we are putting it in Ember's [registry](../../applications/dependency-injection/#toc_factory-registrations) using the [owner](https://api.emberjs.com/ember/3.9/functions/@ember%2Fapplication/getOwner) object given by the test context. When our component loads the maps service, it gets our stub service instead.
