...
[`model`](https://api.emberjs.com/ember/6.5.0/classes/Route/methods/model?anchor=model)
...
Behind the scenes, what is happening is that the [route's controller](https://api.emberjs.com/ember/6.5.0/classes/Route/methods/setupController?anchor=setupController) receives the results of the model hook, and Ember makes the model hook results available to the template. Your app may not have a controller file for the route, but the behavior is the same regardless.
...
Note that EmberData also has a feature called a [`Model`](https://api.emberjs.com/ember-data/5.5.0/classes/Model), but it's a separate concept from a route's [`model`](https://api.emberjs.com/ember/6.5.0/classes/Route/methods/model?anchor=model) hook.
...
[RSVP.hash](https://api.emberjs.com/ember/6.5.0/classes/rsvp/methods/hash?anchor=hash).
...
[`transitionTo`](https://api.emberjs.com/ember/6.5.0/classes/RouterService/methods/transitionTo?anchor=transitionTo)
instead.
...
- Use the [`{{debugger}}`](https://api.emberjs.com/ember/6.5.0/classes/Ember.Templates.helpers/methods/debugger?anchor=debugger) or [`{{log}}`](https://api.emberjs.com/ember/6.5.0/classes/Ember.Templates.helpers/methods/log?anchor=log) helper to inspect the `{{@model}}` from the template
...
- check to see whether the data returned from a `model` hook is an object, array, or JavaScript Primitive. For example, if the result of `model` is an array, using `{{@model}}` in the template won't work. You will need to iterate over the array with an [`{{#each}}`](https://api.emberjs.com/ember/6.5.0/classes/Ember.Templates.helpers/methods/each?anchor=each) helper. If the result is an object, you need to access the individual attribute like `{{@model.title}}` to render it in the template.
