...
To instantiate the service, we can instantiate it through ember's resolver using the [`factoryFor`](https://api.emberjs.com/ember/3.1/classes/ApplicationInstance/methods/factoryFor?anchor=factoryFor) method.
...
For cases where we do not need to override parts of the service, we can use [`lookup`](https://api.emberjs.com/ember/3.1/classes/ApplicationInstance/methods/lookup?anchor=lookup)
...
The location in the cache has been [`camelized`](https://api.emberjs.com/ember/3.1/classes/String/methods/camelize?anchor=camelize) (line 30),
