...
A [Controller](https://api.emberjs.com/ember/3.12/classes/Controller) is routable object which receives a single property from the Route – `model` – which is the return value of the Route's [`model()`](https://api.emberjs.com/ember/3.12/classes/Route/methods?anchor=model) method.
