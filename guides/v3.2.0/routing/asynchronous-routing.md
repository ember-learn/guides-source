...
when the promise rejects, is via the promise's [`then()`](https://api.emberjs.com/ember/3.2/classes/Promise/methods/then?anchor=then) method, which
...
resolved. The values passed to the [`setupController()`](https://api.emberjs.com/ember/3.2/classes/Route/methods/setupController?anchor=setupController) hook for each route
