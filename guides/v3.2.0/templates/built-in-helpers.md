...
The [`{{get}}`](https://api.emberjs.com/ember/3.2/classes/Ember.Templates.helpers/methods/get?anchor=get) helper makes it easy to dynamically send the value of a
...
For example, the [`{{concat}}`](https://api.emberjs.com/ember/3.2/classes/Ember.Templates.helpers/methods/get?anchor=concat) helper makes it easy to dynamically send
...
Now let's say your template is starting to get a bit cluttered and you now want to clean up the logic in your templates. This can be achieved with the `let` block helper. The [`{{let}}`](https://api.emberjs.com/ember/3.2/classes/Ember.Templates.helpers/methods/get?anchor=let) helper lets you create new bindings in your template.
