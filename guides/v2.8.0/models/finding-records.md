...
Use [`store.findRecord()`](https://api.emberjs.com/data/classes/DS.Store.html#method_findRecord) to retrieve a record by its type and ID.
...
Use [`store.peekRecord()`](https://api.emberjs.com/data/classes/DS.Store.html#method_peekRecord) to retrieve a record by its type and ID, without making a network request.
...
Use [`store.findAll()`](https://api.emberjs.com/data/classes/DS.Store.html#method_findAll) to retrieve all of the records for a given type:
...
Calling [`store.query()`](https://api.emberjs.com/data/classes/DS.Store.html#method_query) will make a `GET` request with the passed object serialized as query params.
...
Ember Data provides a convenience method [`store.queryRecord()`](https://api.emberjs.com/data/classes/DS.Store.html#method_queryRecord)that will return a promise that resolves with that single record.
...
then calling [`store.queryRecord()`](https://api.emberjs.com/data/classes/DS.Store.html#method_queryRecord) will retrieve that object from the server:
...
Note that Ember's default [JSON API adapter](https://api.emberjs.com/data/classes/DS.JSONAPIAdapter.html) does not provide the functionality needed to support `queryRecord()` directly as it relies on REST request definitions that return result data in the form of an array.
