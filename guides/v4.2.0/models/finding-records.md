...
Use [`store.findRecord()`](https://api.emberjs.com/ember-data/4.2.0/classes/Store/methods/findRecord?anchor=findRecord) to retrieve a record by its type and ID.
...
Use [`store.peekRecord()`](https://api.emberjs.com/ember-data/4.2.0/classes/Store/methods/peekRecord?anchor=peekRecord) to retrieve a record by its type and ID, without making a network request.
...
Use [`store.findAll()`](https://api.emberjs.com/ember-data/4.2.0/classes/Store/methods/findAll?anchor=findAll) to retrieve all of the records for a given type:
...
Use [`store.peekAll()`](https://api.emberjs.com/ember-data/4.2.0/classes/Store/methods/peekAll?anchor=peekAll) to retrieve all of the records for a given type that are already loaded into the store, without making a network request:
...
It's important to note that `RecordArray` is not a JavaScript array, it's an object that implements [`MutableArray`](https://api.emberjs.com/ember/4.2.0/classes/MutableArray).
...
Calling [`store.query()`](https://api.emberjs.com/ember-data/4.2.0/classes/Store/methods/query?anchor=query) will make a `GET` request with the passed object serialized as query params.
...
Ember Data provides a convenience method [`store.queryRecord()`](https://api.emberjs.com/ember-data/4.2.0/classes/Store/methods/queryRecord?anchor=queryRecord) that will return a promise that resolves with that single record.
...
Then, calling [`store.queryRecord()`](https://api.emberjs.com/ember-data/4.2.0/classes/Store/methods/queryRecord?anchor=queryRecord) will retrieve that object from the server:
...
Note that Ember's default [JSON:API adapter](https://api.emberjs.com/ember-data/4.2.0/classes/JSONAPIAdapter) does not provide the functionality needed to support `queryRecord()` directly as it relies on REST request definitions that return result data in the form of an array.
