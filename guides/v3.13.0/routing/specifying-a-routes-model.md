...
[`model`](https://api.emberjs.com/ember/3.13/classes/Route/methods/model?anchor=model)
...
In the next example, we will use Ember Data's [`findAll`](https://api.emberjs.com/ember-data/3.13/classes/Store/methods/findAll?anchor=findAll) method, which returns a Promise, and resolves with an array of [Ember Data records](../../models/).

_Note that Ember Data also has a feature called a [`Model`](https://api.emberjs.com/ember-data/3.13/classes/Model), but it's a separate concept from a route's [`model`](https://api.emberjs.com/ember/3.13/classes/Route/methods/model?anchor=model) hook._
...
[RSVP.hash](https://api.emberjs.com/ember/3.13/classes/rsvp/methods/hash?anchor=hash).
...
[`transitionTo`](https://api.emberjs.com/ember/3.13/classes/RouterService/methods/transitionTo?anchor=transitionTo)
instead.
...
- Use the [`{{debugger}}`](https://api.emberjs.com/ember/3.13/classes/Ember.Templates.helpers/methods/debugger?anchor=debugger) or [`{{log}}`](https://api.emberjs.com/ember/3.13/classes/Ember.Templates.helpers/methods/log?anchor=log) helper to inspect the `{{model}}` from the template
...
- check to see whether the data returned from a `model` hook is an object, array, or JavaScript Primitive. For example, if the result of `model` is an array, using `{{this.model}}` in the template won't work. You will need to iterate over the array with an [`{{#each}}`](https://api.emberjs.com/ember/3.13/classes/Ember.Templates.helpers/methods/each?anchor=each) helper. If the result is an object, you need to access the individual attribute like `{{this.model.title}}` to render it in the template.
