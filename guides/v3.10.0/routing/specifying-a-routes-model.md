...
[`model`](https://api.emberjs.com/ember/3.10/classes/Route/methods/model?anchor=model)
...
_Note that Ember Data also has a feature called a [`Model`](https://api.emberjs.com/ember-data/3.10/classes/DS.Model), but it's a separate concept from a route's [`model`](https://api.emberjs.com/ember/3.10/classes/Route/methods/model?anchor=model) hook._
...
[RSVP.hash](https://api.emberjs.com/ember/3.10/classes/rsvp/methods/hash?anchor=hash).
...
[`transitionTo`](https://api.emberjs.com/ember/3.10/classes/RouterService/methods/transitionTo?anchor=transitionTo)
instead.
...
- check to see whether the data returned from a `model` hook is an object, array, or JavaScript Primitive. For example, if the result of `model` is an array, using `{{this.model}}` in the template won't work. You will need to iterate over the array with an [`{{#each}}`](https://api.emberjs.com/ember/3.10/classes/Ember.Templates.helpers/methods/each?anchor=each) helper. If the result is an object, you need to access the individual attribute like `{{this.model.title}}` to render it in the template.
