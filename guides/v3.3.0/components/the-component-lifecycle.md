...
After a component successfully renders its backing HTML element into the DOM, it will trigger its [`didInsertElement()`](https://api.emberjs.com/ember/3.3/classes/Component/events/didInsertElement?anchor=didInsertElement) hook.
...
2. The component's element is accessible via the component's
   [`$()`](https://api.emberjs.com/ember/3.3/classes/Component/methods/$?anchor=%24)
   method.

A component's [`$()`](https://api.emberjs.com/ember/3.3/classes/Component/methods/$?anchor=%24) method allows you to access the component's DOM element by returning a jQuery element.
...
[`$()`](https://api.emberjs.com/ember/3.3/classes/Component/methods/$?anchor=%24) will, by default, return a jQuery object for the component's root element, but you can also target child elements within the component's template by passing a selector:
...
Let's initialize our date picker by overriding the [`didInsertElement()`](https://api.emberjs.com/ember/3.3/classes/Component/events/didInsertElement?anchor=didInsertElement) method.
...
[`didInsertElement()`](https://api.emberjs.com/ember/3.3/classes/Component/events/didInsertElement?anchor=didInsertElement) is also a good place to
...
- Setting properties on the component in [`didInsertElement()`](https://api.emberjs.com/ember/3.3/classes/Component/events/didInsertElement?anchor=didInsertElement) triggers a re-render, and for performance reasons,
  is not allowed.
- While [`didInsertElement()`](https://api.emberjs.com/ember/3.3/classes/Component/events/didInsertElement?anchor=didInsertElement) is technically an event that can be listened for using `on()`, it is encouraged to override the default method itself,
...
When a component detects that it is time to remove itself from the DOM, Ember will trigger the [`willDestroyElement()`](https://api.emberjs.com/ember/3.3/classes/Component/events/willDestroyElement?anchor=willDestroyElement) method,
