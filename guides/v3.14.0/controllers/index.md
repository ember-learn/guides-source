...
A [Controller](https://api.emberjs.com/ember/3.14/classes/Controller) is routable object which receives a single property from the Route – `model` – which is the return value of the Route's [`model()`](https://api.emberjs.com/ember/3.14/classes/Route/methods/model?anchor=model) method.
