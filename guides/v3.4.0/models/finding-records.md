...
Use [`store.peekAll()`](https://api.emberjs.com/ember-data/release/classes/DS.Store/methods/findAll?anchor=peekAll) to retrieve all of the records for a given type that are already loaded into the store, without making a network request:
